...
def bareCatData(cat_list, cat_qdata):
    ...
    return cat_fn_list, cat_energy_list, active_site_chelpg_list


cat_dir, catO2_dir = sys.argv[1].rstrip('/'), sys.argv[2].rstrip('/') # directories containing .out files

catO2_fn = catO2_dir.split('.')[0].split('/')[-1]
cat_fn = cat_dir.split('.')[0].split('/')[-1]

cation_dir = sys.argv[3]
cation_fn = cation_dir.split('.')[0].split('/')[-1]
...
pickle_qout.pickle_data(cation_dir, cation_fn + ".p")
...
cation_qdata = pickle.load(open(cation_fn + ".p", "rb"))
...
print(cat_list)
cat_fn_list, cat_energy_list, active_site_chelpg_list = bareCatData(cat_list, cat_qdata)
cation_fn_list, cation_energy_list, cation_site_chelpg_list = bareCatData(cat_list, cation_qdata)
...
matched_df['Cation_Energy'] = cation_energy_list
matched_df['CationAS_Bare_CHELPG'] = cation_site_chelpg_list
